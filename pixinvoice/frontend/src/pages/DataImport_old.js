...
import { Upload, Download, FileText, Users, UserPlus, CheckCircle, AlertCircle } from 'lucide-react';
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (activeTab === 'customers') {
        ...
        const response = await importAPI.importCustomers(formData);
        setImportResult(response.data);
        
        // Build success message
        let successMsg = `${response.data.total || (response.data.created + response.data.updated)} cég importálva`;
        if (response.data.nav_found > 0) {
          successMsg += `, ebből ${response.data.nav_found} NAV-ból frissítve`;
        }
      ...
      toast.error(error.response?.data?.error || 'Hiba az importálás során');
      setImportResult({
        success: false,
        error: error.response?.data?.error || 'Ismeretlen hiba'
      });
    } finally {
      setImporting(false);
    }
    ...
    <Container>
      ...
      {activeTab === 'customers' && (
        <Card>
          ...
          <ButtonGroup>
            <Button variant="primary" onClick={handleImport} disabled={importing || !customerFile}>
              <Upload size={18} />
      ...
      {activeTab === 'contacts' && (
        <Card>
          ...
          <ButtonGroup>
            <Button variant="primary" onClick={handleImport} disabled={importing || !contactFile}>
              <Upload size={18} />
              {importing ? 'Importálás folyamatban...' : 'Importálás'}
            </Button>
