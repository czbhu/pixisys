...
class BaseRequestXml:
    ...
    def __init__(self, config: NavApiConfig):
        ...
        """
        self.config = config
        # NAV API maximum 30 karaktert fogad el requestId-re, csak alfanumerikus karakterek és aláhúzás
        import random
        import string
        self.request_id = ''.join(random.choices(string.ascii_letters + string.digits + '_', k=30))
...
class QueryTaxpayerRequestXml(BaseRequestXml):
    """QueryTaxpayer kérés XML"""
    
    root_name = "QueryTaxpayerRequest"
    
    def __init__(self, config: NavApiConfig, taxpayer_tax_number: str):
        """
        QueryTaxpayer kérés inicializálása
        
        Args:
            config: NAV API konfiguráció
            taxpayer_tax_number: Keresett adószám
        """
        self.taxpayer_tax_number = taxpayer_tax_number
        super().__init__(config)
    
    def _create_xml(self) -> ET.Element:
        """XML objektum létrehozása"""
        root = super()._create_xml()
        self._add_taxpayer_data(root)
        return root
    
    def _add_taxpayer_data(self, root: ET.Element):
        """Taxpayer adatok hozzáadása"""
        tax_number = ET.SubElement(root, "taxNumber")
        tax_number.text = self.taxpayer_tax_number
