...
class Machine(models.Model):
    """Nyomtató/feldolgozó gép berendezés."""
    TECH_TYPE_CHOICES = [
        ('uv_flat', 'UV Flatbed'),
        ('uv_roll', 'UV Tekercses'),
        ('digital_sheet', 'Digitális ívnyomtatás'),
        ('digital_roll', 'Digitális tekercs'),
        ('offset', 'Ofszet'),
        ('other', 'Egyéb'),
    ]
    name = models.CharField(max_length=200, verbose_name="Gép neve")
    tech_type = models.CharField(
        max_length=30, choices=TECH_TYPE_CHOICES, default='uv_flat', verbose_name="Technológia"
    )
    max_width_mm = models.DecimalField(
        max_digits=8, decimal_places=2, null=True, blank=True, verbose_name="Max szélesség (mm)"
    )
    max_height_mm = models.DecimalField(
        max_digits=8, decimal_places=2, null=True, blank=True, verbose_name="Max magasság (mm)"
    )
    print_cost_per_m2 = models.DecimalField(
        max_digits=10, decimal_places=4, default=0, verbose_name="Nyomtatási költség / m²"
    )
    hourly_cost = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name="Óradíj"
    )
    setup_time_min = models.DecimalField(
        max_digits=6, decimal_places=2, default=0, verbose_name="Beállítási idő (perc)"
    )
    is_active = models.BooleanField(default=True, verbose_name="Aktív")
