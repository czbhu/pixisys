...
interface ServiceSupplierPricesProps {
    visible: boolean;
    serviceId: number;
    serviceName: string;
    onClose: () => void;
...
const ServiceSupplierPrices: React.FC<ServiceSupplierPricesProps> = ({ 
    visible, 
    serviceId, 
    serviceName, 
    onClose 
    ...
    const fetchSupplierPrices = async () => {
        setLoading(true);
        try {
            const response = await api.get(`/manufacturing/service-supplier-prices/?service_id=${serviceId}`);
    ...
    const handleDelete = async (id: number) => {
        try {
            await api.delete(`/manufacturing/service-supplier-prices/${id}/`);
    ...
    const handleSubmit = async (values: any) => {
        try {
            if (editingPrice) {
                await api.patch(`/manufacturing/service-supplier-prices/${editingPrice.id}/`, values);
                message.success('Beszállítói ár frissítve');
            } else {
                await api.post('/manufacturing/service-supplier-prices/', values);
    ...
    return (
        <>
            <Modal
                title={`Beszállítói árak - ${serviceName}`}
                open={visible}
                onCancel={onClose}
                width={1200}
                footer={[
...
export default ServiceSupplierPrices;
