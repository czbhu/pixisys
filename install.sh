...
if [ ! -f ".env" ]; then
    ...
    cat > .env <<EOF
# PixiERP Frontend .env (Generated by install.sh)
...
CSRF_TRUSTED_ORIGINS=$INV_CSRF_TRUSTED
CORS_ALLOWED_ORIGINS=$INV_CORS_ALLOWED
...
if [ ! -f ".env" ]; then
    ...
    cat > .env <<EOF
# PixInvoice Frontend .env (Generated by install.sh)
REACT_APP_API_URL=
...
EOF
        ...
        if [ "$INSTALL_NGINX" = "i" ] || [ "$INSTALL_NGINX" = "I" ]; then
            ...
            if sudo nginx -t; then
                ...
                # SSL tanúsítvány kérés
                if [ "$USE_HTTPS" = "true" ]; then
                    echo ""
                    echo -e "${BLUE}🔐 SSL tanúsítvány kérés...${NC}"
                    echo "A Let's Encrypt SSL tanúsítvánnyal biztonságos HTTPS kapcsolatot hozhatsz létre."
                    echo ""
...
# Nginx konfiguráció szimbolikus linkek létrehozása (ha nginx van)
if command -v nginx &> /dev/null; then
    if [ -f "$SCRIPT_DIR/nginx/${ERP_DOMAIN_NAME}.conf" ] || [ -f "$SCRIPT_DIR/nginx/${INV_DOMAIN_NAME}.conf" ]; then
        echo ""
        echo -e "${BLUE}🔗 Nginx szimbolikus linkek létrehozása...${NC}"
        ...
        if sudo nginx -t 2>/dev/null; then
            ...
            echo -e "${YELLOW}⚠️  Nginx teszt sikertelen${NC}"
        fi
    fi
fi
